...
@section('content')
    <div id="wrapper">
        <div class="content">
            <div class="row">
                <div class="col-md-12">
                    <div class="_buttons">
                        <a onclick="addCategoriesParent(event)" class="btn btn-primary mright5 test pull-left display-block">
    ...
    @include('template.modal', ['id' => 'showDetail_Modal', 'title'=>'Form Categories Parent', 'form'=>'categories-parent.create'])
@endsection
@push('script')
    {{ $dataTable->scripts() }}
    <script>
        ...
        function dialogConfirmWithAjaxCategoriesParent(sureCallbackFunction, data, text = "Xin hay kiểm tra lại") {
            Swal.fire({
        ...
        function changeStatusCategoriesParent(data){
            ...
            $.post('/categories-parent/change-status', dataPost).done(function(response) {
                alert_float('success', response.message);
                $('#categories_manage').DataTable().ajax.reload(null,false);
            });
        }
    
        function deleteCategoriesParent(data) {
            let dataPost = {};
            dataPost.id = $(data).data('id');
            $.post('/categories-parent/destroy', dataPost).done(function (response) {
                alert_float('success', response.message);
                let table = $('#categories_manage').DataTable();
                table.ajax.reload(null, false);
            });
        ...
        function addCategoriesParent(e) {
                e.preventDefault();
                ...
                window.urlMethod = '/categories-parent/store'; // Thiết lập phương thức URL cho thêm mới
                window.type = 'POST'; // Thiết lập phương thức HTTP
            }


        function detailCategoriesParent(_this) {
    let dataPost = {};
    dataPost.id = $(_this).data('id');
    $.post('/categories-parent/show', dataPost).done(function (response) {
        ...
        $('#showDetail_Modal').modal('toggle');
        window.urlMethod = '/categories-parent/update/' + $(_this).data('id');
        window.type = 'PUT';
    });
}


        function pushCategoriesParent() {
